...
// CORS helpers
const allowedOriginPatterns = [/\.netlify\.app$/, /\.vercel\.app$/];
...
async function startServer() {
  ...
  apiRouter.delete("/posts/:id", authenticateToken, (req: AuthRequest, res) => {
    try {
      ...
      const post: any = db.prepare("SELECT * FROM posts WHERE id = ?").get(postId);
    ...
    try {
      ...
      if (!startup_name?.trim() || !launched_date?.trim() || !pitch?.trim()) {
        return res.status(400).json({
          error: "Startup name, launch date, and pitch required",
        });
      }
