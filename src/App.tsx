  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        onLogin(userCredential.user as any);
        navigate("/");
      } else {
    ...
    <div className="max-w-2xl mx-auto py-12 px-4">
      ...
      <div className="mt-10">
          ...
          <div className="space-y-6">
            {posts.map((post) => (
              <PostCard
                ...
                onLike={() => {}}
                onDelete={() => {}}
                onEdit={async (_id: number, _content: string) => {}}
                onReport={() => {}}
                onCommentClick={() => {}}
                onCoBuild={() => {}}
              />
        ...
        {isMessageOpen &&
          createPortal(
            <div
              className="fixed inset-0 z-[9999] bg-black/50 flex items-center justify-center p-4 w-screen h-screen"
              onClick={() => setIsMessageOpen(false)}
            >
              <div
                className="bg-white dark:bg-zinc-900 w-[95%] sm:w-full max-w-sm sm:max-w-md rounded-2xl p-5 sm:p-6 shadow-xl max-h-[75vh] sm:max-h-[90vh] overflow-y-auto"
